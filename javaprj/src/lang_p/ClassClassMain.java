...
public class ClassClassMain {
	...
	public static void main(String[] args) {
		...
		// 멤버변수
		System.out.println("getDeclaredFields: "+Arrays.toString(clzz.getDeclaredFields()));
		
		// 메소드의 선언부만 보임
		System.out.println("getDeclaredMethods: "+Arrays.toString(clzz.getDeclaredMethods()));
		
		// 생성자 (선언만 보임)
		System.out.println("getDeclaredConstructors: "+Arrays.toString(clzz.getDeclaredConstructors()));
		
		// aa가 null이라 에러 발생
//		Class aa = null;
//		aa.getResource("fff/dog1.jpg");
		// static이 아님
//		Class.getResource("fff/dog1.jpg");
		
		// getResource - 파일 URL 리턴
		// ClAAA와 파일이 관련 없어도 Class 객체를 생성하여 메소드 사용
		URL uu = clzz.getResource("../fff/dog1.jpg");
...
//			Class nn = Class.forName("lang_p.ClAA"); 존재하지 않는 클래스명 기재시 에러 발생
...
//			Object oo3 = nn.newInstance(300, "나는상어"); 매개변수를 직접 넣어 처리 불가
			// 생성자 가져오기
			Constructor con2 = nn.getConstructor(int.class, String.class);
//			Object oo4 = con.newInstance(400, "나는고래");
...
//			ClAAA(int a) 가 public으로 선언되지 않아서 에러 발생
//			Constructor con1 = nn.getConstructor(int.class);
			...
			System.out.println("fc: "+fc);  // private int lang_p.ClAAA.c
			// private이라 접근 불가
//			System.out.println("fc.get(a5): "+fc.get(a5));
			System.out.println("fd: "+fd);  // private int lang_p.ClAAA.d
			fd.setAccessible(true);	// private을 접근 가능토록 변경
			System.out.println("fd.get(a5): "+fd.get(a5));  // 40
//			System.out.println("a5.d: "+a5.d);  직접 접근 불가
			
			a5.meth_1();  // meth_1 실행: 1234, 캐롤인가, 30, 40
			// 대입
...
//			fc.set(a5, 369);  private 에러
			// fd.setAccessible(true); private을 접근 가능토록 변경
			fd.set(a5, 480);
			a5.meth_1();  // meth_1 실행: 5678, 흰눈사이로, 30, 480
