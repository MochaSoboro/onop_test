...
public class ReduceMain {
	public static void main(String[] args) {
		...
		arr = new int[] {137};
		double dd = Arrays.stream(arr).findFirst().orElse(0);
		System.out.println("orElse: "+dd);
...
//		System.out.println("reduce 초기값: "+rr);
		...
		int [] numArr = {13,66,45,2,78,9,12,19,88,34};
		rr = Arrays.stream(numArr).reduce(0, (a, b) -> {
			int sum = b%2 == 0 ? a+b : a;
			return sum;
		});
		System.out.println(rr);
		
		rr = Arrays.stream(numArr).filter(a -> a%2 == 0).sum();
		System.out.println(rr);
	}
}
